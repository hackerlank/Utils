...
TEST(FilePath, AbsolutePath) {
    ...
    EXPECT_FALSE(absolute_path("a/b/c", NULL, buf, sizeof(buf)));
    EXPECT_FALSE(absolute_path("a/b/c", "1.txt", NULL, sizeof(buf)));
    EXPECT_FALSE(absolute_path("a/b/c", "../../1.txt", buf, sizeof(buf)));

    // 无需计算绝对路径直接返回
#ifdef OS_WIN
    EXPECT_TRUE(absolute_path("a/b/c", "C:/1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("C:/1.txt", buf);
#else
    EXPECT_TRUE(absolute_path("a/b/c", "/1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("/1.txt", buf);
#endif

    // 相对于当前工作目录
    char buf1[MAX_PATH];
    snprintf(buf1, sizeof(buf1), "%s%s", get_current_dir(), "1.txt");
    EXPECT_TRUE(absolute_path(NULL, "1.txt", buf, sizeof(buf)));
    EXPECT_STREQ(buf1, buf);

    char buf2[MAX_PATH];
    EXPECT_TRUE(path_find_directory(get_execute_dir(), buf2, sizeof(buf2)));
    snprintf(buf1, sizeof(buf1), "%s%s", buf2, "1.txt");
    EXPECT_TRUE(absolute_path(NULL, "../1.txt", buf, sizeof(buf)));
    EXPECT_STREQ(buf1, buf);

    EXPECT_TRUE(absolute_path("", "1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("1.txt", buf);
    EXPECT_FALSE(absolute_path("", "../1.txt", buf, sizeof(buf)));

    EXPECT_TRUE(absolute_path("a/b/c", "1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("a/b/1.txt", buf);
    EXPECT_TRUE(absolute_path("a/b/c/", "1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("a/b/c/1.txt", buf);

    EXPECT_TRUE(absolute_path("a/b/c", "./1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("a/b/1.txt", buf);
    EXPECT_TRUE(absolute_path("a/b/c", "../1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("a/1.txt", buf);
    EXPECT_TRUE(absolute_path("/a/b/c", "../../1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("/1.txt", buf);
    EXPECT_TRUE(absolute_path("/a/b/c", "./.././.././1.txt", buf, sizeof(buf)));
    EXPECT_STREQ("/1.txt", buf);
